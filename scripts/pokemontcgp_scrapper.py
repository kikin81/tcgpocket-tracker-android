...
packs = ['Pikachu pack', 'Charizard pack', 'Mewtwo pack', 'Mythical Island pack']
...
def extract_id(soup):
    """
    Extracts a unique card ID based on the set name and card number from the URL.

    Args:
        soup: BeautifulSoup object containing the HTML of the card page.

    Returns:
        str: A unique card ID in the format 'SetNameCardID' (e.g., 'A1a086').
    """
    title = soup.find('p', class_='card-text-title')
    href = title.find('a')['href']  # Extract the href attribute, e.g., "/cards/A1a/86"
    parts = href.split('/')         # Split the URL path into components

    # Extract the set name (second part) and card ID (last part)
    set_name = parts[2]             # "A1a"
    card_id = parts[-1].zfill(3)    # "86" padded to "086"

    # Combine the set name and padded card ID
    unique_id = f"{set_name}{card_id}"
...
def convert_cards_to_json(set_id, filename):
    """
    Converts cards from the specified set to JSON.

    Args:
        set_id (str): The ID of the set (e.g., 'A1', 'P-A', 'A1a').
        filename (str): The output filename for the JSON file.
    """
    if set_id not in sets:
        raise ValueError(f"Set ID {set_id} not found in defined sets.")

    # Retrieve card range for the set
    cards_in_set = sets[set_id]
    start_id = 1
    end_id = cards_in_set
...
# Convert cards from set 'A1'
convert_cards_to_json('A1', 'genetic_apex_cards.json')
end_time = time.time()
print(f'Datos guardados en genetic_apex_cards.json, tiempo total: {end_time - init_time} segundos.')

# Convert cards from set 'P-A'
init_time = time.time()
convert_cards_to_json('P-A', 'promo_a_cards.json')
end_time = time.time()
print(f'Datos guardados en promo_a_cards.json, tiempo total: {end_time - init_time} segundos.')

# Convert cards from set 'A1a'
init_time = time.time()
convert_cards_to_json('A1a', 'mythical_islands_cards.json')
end_time = time.time()
print(f'Datos guardados en mythical_islands_cards.json, tiempo total: {end_time - init_time} segundos.')
# iterate_per_set('A1', 1, 286)
# iterate_per_set('P-A', 1, 33)
# iterate_per_set('A1a', 1, 86)
# iterate_all_sets()
